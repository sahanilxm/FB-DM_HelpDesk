...
const AgentScreen = () => {
  ...
  const fetchMessages = async () => {
    try {
      ...
      const API_URI = `https://graph.facebook.com/v19.0/${PAGE_ID}/conversations?fields=participants,messages%7Bid,message,created_time,from%7D&access_token=${PAGE_ACCESS_TOKEN}`;
  ...
  const sendMessage = async () => {
    try {
      ...
      const API_URI = `https://graph.facebook.com/v19.0/me/messages?access_token=${PAGE_ACCESS_TOKEN}`;
      const res = await fetch(API_URI,
        {
          method: "POST",
          headers: {
  ...
  const checkIfNewMessage = async () => {
    try {
      const API_URI = `${SERVER_URL}/lastMessageTimestamp?custId=${activeChatMessages.id}`;
      const res = await fetch(API_URI,
        {
          method: "GET",
          headers: {
    ...
    if (isNewMessage) {
      try {
        ...
        if (newMessages.length > 0) {
          const res = await fetch(`${SERVER_URL}/storeMessage`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
